...
if __name__ == "__main__":
        ...
        for idx, (duration, hash) in enumerate(zip(durations, hashes)):
            ...
            if plot:
                if duration == 0:
                    ...
                    normalized_durations.append(min(normalized_duration, ymax))
    ...
    average_lines = plt.plot(average, label="Average", linewidth=1.5)
    average_color = average_lines[0].get_color()
    plt.annotate("{:.2f}".format(average[0]), xy=(-1.4, average[0]), fontsize=9, color=average_color, zorder=2)
    plt.annotate("{:.2f}".format(average[-1]), xy=(iteration_count - 1, average[-1]), fontsize=9, color=average_color, zorder=2)
    #plt.plot(median, label="Median", linewidth=1.5)

    plt.xlabel("Iteration Index")
    plt.ylabel("Runtime relative to {}".format(reference_duration_type))
    plt.legend()
    plt.axhline(y=1, linewidth=1.5)
    plt.ylim(ymin=0.8, ymax=ymax)
    plt.xticks(range(0, iteration_count))
    plt.xlim(xmin=-1.7, xmax=iteration_count+0.5)
    #plt.show()
    plt.savefig("adaptive_optimization_runtimes-relative-to-{}-{}.png".format(reference_duration_type, evaluation_name))
