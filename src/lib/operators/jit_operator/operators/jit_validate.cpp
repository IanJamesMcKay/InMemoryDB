...
void JitValidate<use_ref_pos_list>::_consume(JitRuntimeContext& context) const {
  ...
  if constexpr (use_ref_pos_list) {
    ...
    row_is_visible = jit_is_row_visible(context.transaction_id, context.snapshot_commit_id, row_id.chunk_offset, *mvcc_columns);
  } else {
    row_is_visible = jit_is_row_visible(context.transaction_id, context.snapshot_commit_id, context.chunk_offset, *context.columns);
  }
