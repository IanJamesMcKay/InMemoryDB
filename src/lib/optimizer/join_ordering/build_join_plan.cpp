...
template<typename ResolveColumnIDFn>
JoinPlanPredicateEstimate estimate_predicate(const AbstractJoinPlanPredicate& predicate,
const std::shared_ptr<TableStatistics>& statistics, ResolveColumnIDFn resolve_column_id_fn) {
  switch (predicate->type()) {
    ...
    case JoinPlanPredicateType::LogicalOperator: {
      ...
      const auto left_operand_estimate = estimate_predicate(*logical_operator_predicate->left_operand, statistics, resolve_column_id_fn);
      ...
      switch (logical_operator_predicate->logical_operator) {
        case JoinPlanPredicateLogicalOperator::And: {
          ...
          estimate_predicate(*logical_operator_predicate->right_operand, left_operand_estimate.statistics, resolve_column_id_fn);
        ...
        case JoinPlanPredicateLogicalOperator::Or: {
          const auto right_operand_estimate =
          estimate_predicate(*logical_operator_predicate->right_operand, statistics, resolve_column_id_fn);
...
void order_predicates(const AbstractJoinPlanNode& join_plan_node, std::vector<std::shared_ptr<const AbstractJoinPlanPredicate>>& predicates,
                                             const std::shared_ptr<TableStatistics>& statistics) {
  const auto sort_predicate = [&](auto& left, auto& right) {
    return estimate_predicate(join_plan_node, left, statistics).statistics->row_count() <
           estimate_predicate(join_plan_node, right, statistics).statistics->row_count();
  };
...
std::shared_ptr<JoinPlanJoinNode> build_join_plan_join_node(
   ...
   */
  const auto iter = std::find_if(predicates.begin(), predicates.end(), [&](const auto& predicate) {
  ...
  order_predicates(secondary_predicates, statistics);

  // Apply remaining predicates
  for (const auto& predicate : secondary_predicates) {
    const auto predicate_estimate = estimate_predicate(predicate, statistics, cost_model, [&](const auto& column_reference) {
  ...
  return std::make_shared<JoinPlanJoinNode>(left_child, right_child, primary_join_predicate, secondary_predicates,
                                            node_cost);
}
...
std::shared_ptr<JoinPlanVertexNode> build_join_plan_vertex_node(
  ...
  const auto statistics = vertex_node->get_statistics();

  const auto find_output_column_id = [&](const auto& column_reference) {
    return vertex_node->find_output_column_id(column_reference);
  };

  order_predicates(predicates, statistics, find_output_column_id);

  auto node_cost = 0.0f;

  for (const auto& predicate : predicates) {
    const auto predicate_estimate = estimate_predicate(predicate, statistics, find_output_column_id);
