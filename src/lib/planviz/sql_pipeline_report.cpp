...
void create_sql_pipeline_report(SQLPipeline& sql_pipeline, const std::string &name, const GraphvizConfig& graphviz_config, const VizGraphInfo& graph_info, const VizVertexInfo& vertex_info,
  ...
  LQPVisualizer{graphviz_config, graph_info, vertex_info, edge_info}.visualize(unoptimized_lqps, name + ".raw_lqp.dot", name + ".raw_lqp" + extension);

  const auto optimized_lqps = sql_pipeline.get_optimized_logical_plans();
  LQPVisualizer{graphviz_config, graph_info, vertex_info, edge_info}.visualize(optimized_lqps, name + ".opt_lqp.dot", name + ".opt_lqp" + extension);
  ...
  for (size_t sql_query_plan_idx{0}; sql_query_plan_idx < sql_query_plans.size(); ++sql_query_plan_idx) {
    ...
    SQLQueryPlanVisualizer{graphviz_config, graph_info, vertex_info, edge_info}.visualize(*sql_query_plans[sql_query_plan_idx], prefix + ".dot", prefix + extension);
  }
  ...
  for (size_t lqp_idx{0}; lqp_idx < unoptimized_lqps2.size(); ++lqp_idx) {
    ...
    JoinPlanVisualizer{graphviz_config, graph_info, vertex_info, edge_info}.visualize(join_plan, prefix_join_plan + ".dot", prefix_join_plan + extension);
  }
}
