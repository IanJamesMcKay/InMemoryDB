...
template <>
class EqualNumElementsHistogram<std::string> : public AbstractStringHistogram {
 public:
  using AbstractStringHistogram::AbstractStringHistogram;
 ...
 protected:
  ...
  size_t _num_buckets() const override;
  BucketID _bucket_for_value(const std::string& value) const override;
  BucketID _lower_bound_for_value(const std::string& value) const override;
  BucketID _upper_bound_for_value(const std::string& value) const override;

  uint64_t _bucket_min(const BucketID index) const override;
  uint64_t _bucket_max(const BucketID index) const override;
  uint64_t _bucket_count(const BucketID index) const override;
  uint64_t _total_count() const override;
