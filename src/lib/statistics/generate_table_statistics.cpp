...
TableStatistics generate_table_statistics(const Table& table, const size_t sample_count_hint) {
  ...
  for (ColumnID column_id{0}; column_id < table.column_count(); ++column_id) {
    ...
    resolve_data_type(column_data_type, [&](auto type) {
      ...
      column_statistics.emplace_back(generate_column_statistics<ColumnDataType>(table, column_id, sample_count_hint));
    });
  }
