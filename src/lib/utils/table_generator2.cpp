...
struct UniformDistribution<int32_t> {
  using type = std::uniform_int_distribution<int32_t>;
};
template <>
struct UniformDistribution<int64_t> {
  using type = std::uniform_int_distribution<int64_t>;
};

TableGenerator2::TableGenerator2(const TableGenerator2ColumnDefinitions& column_definitions,
                                 const size_t num_rows_per_chunk, const size_t chunk_count)
  ...
  for (ChunkID chunk_id{0}; chunk_id < _chunk_count; ++chunk_id) {
    ...
    for (ColumnID column_id{0}; column_id < _column_definitions.size(); ++column_id) {
      resolve_data_type(_column_definitions[column_id].data_type, [&](auto type) {
        ...
        if
          constexpr(std::is_integral_v<ColumnDataType> || std::is_floating_point_v<ColumnDataType>) {
