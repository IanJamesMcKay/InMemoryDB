...
class CostFeatureProxyTest : public ::testing::Test {
 public:
  ...
  Proxies get_proxies() const {
    // clang-format off
    if constexpr (std::is_same_v<ProxyType, CostFeatureJoinPlanProxy>) {
      ...
      return Proxies{std::make_shared<CostFeatureJoinPlanProxy>(join),
                     std::make_shared<CostFeatureJoinPlanProxy>(predicate)};
    } else {
...
typedef ::testing::Types<CostFeatureJoinPlanProxy, CostFeatureOperatorProxy> ProxyTypes;
...
TYPED_TEST(CostFeatureProxyTest, AllFeatures) {
  ...
  EXPECT_FLOAT_EQ(proxies.join_proxy->extract_feature(CostFeature::LeftInputRowCountLogN).scalar(), 150 * std::log(150));
  ...
  EXPECT_EQ(proxies.join_proxy->extract_feature(CostFeature::PredicateCondition).predicate_condition(), PredicateCondition::Equals);
  EXPECT_EQ(proxies.predicate_proxy->extract_feature(CostFeature::PredicateCondition).predicate_condition(), PredicateCondition::NotEquals);
